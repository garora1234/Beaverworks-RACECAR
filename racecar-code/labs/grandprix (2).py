...
CONES_BLUE = ((100, 150, 150), (120, 255, 255))  
CONES_RED = ((170, 50, 50), (10, 255, 255)) 
...
def follow_art_wall():
    ...
    angle = rc_utils.clamp(dist_diff * 0.01, -1, 1)
    
    scan = (scan - 0.01) % 1000000


    for i in range(2): 
        if rc_utils.get_lidar_average_distance(scan,-88 + i,2) > 600 or rc_utils.get_lidar_average_distance(scan,88 - i,2) > 600 and  center_distance > 40:
...
def follow_line():
    ...
    global line_detected
    if not contour_center is None:
        angle = rc_utils.clamp(rc_utils.remap_range(contour_center[1] - 50, 0, rc.camera.get_width(), -1, 1), -1, 1)
        speed = 1
        line_detected = True
