...
def update():
    ...
    left_dist = rc_utils.get_lidar_average_distance(scan, 270, 30)
    ...
    if slope > 0.01:
        pass_time = SLOPE_PASS_TIME
    else:
        pass_time = SHORT_PASS_TIME
    ...
    if cur_mode == Mode.red_pass:
        ...
        if counter <= 0 and 0 < left_dist < 1000:
        ...
        if counter <= 0 and 0 < right_dist < 1000:
    ...
    print(
        f"Mode: {cur_mode.name}, left_distance: {left_dist:.2f} cm, right_distance: {right_dist:.2f} cm, red_distance: {red_distance:.2f} cm, blue_distance: {blue_distance:.2f} cm, speed: {speed:.2f}, angle: {angle:2f}"
    )
