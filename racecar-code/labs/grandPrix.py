...
def update_line_contour():
    ...
    if imageCopy is None:
        contour_center = None
        contour_area = 0
    elif COLOR[0] is None or COLOR[1] is None:
        contour_center = None
        contour_area = 0
    else:
...
def follow_art_wall():
    ...
    print(distance)
    print(distance2)
    if distance > 180 and distance2 > 180:
        angle = -1

    if ids is not None and ids[0][0] == 199 and distanceToTag < 90:
...
def update():
    ...
    if ids is not None:
        ...
        elif[0] in ids and distance < 140:
            update_ar_contour()
